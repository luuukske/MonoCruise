...
| Fix D target arc over-rotation | When `fwd_dot` in `(-0.5, 0.7)` + `\|v_curvature\| > _TURNING_DIVERGE_CURVATURE` + `speed > 0.5` + heading rotation over horizon (`\|v_curvature\| × arc_length`) exceeds angle to anti-parallel alignment (`acos(-fwd_dot)`) + direction guard (`cross(veh_fwd, -ego_fwd) × v_curvature < 0`): scale `arc_curvature = v_curvature / _TURN_COMPLETE_CURVATURE_SCALE (3.0)`. Applied in `_build_vehicle_collision_data` (precompute path) and `pc is None` fallback. `v_curvature` itself is unchanged — `same_curve` and other checks still use raw curvature. Mirror of ego evasion centerline-snap. |
...
- **Fix D applies to `arc_curvature`, not `v_curvature`.** `v_curvature` is the raw measured curvature used by `same_curve`, `own_lane`, and `same_curve` checks — do not dampen it. Only the curvature passed to `build_arc()` / `get_arc(curvature_override=...)` is scaled down.
- **Fix D direction guard uses `cross(veh_fwd, -ego_fwd) × v_curvature < 0`.** This detects rotation toward anti-parallel. A vehicle rotating toward parallel (U-turn into ego's lane) has opposite sign product and is preserved as a real threat.
