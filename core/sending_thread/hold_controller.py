...
class HoldController:
    ...
    def _hold_pedal(
        ...
        ff_required *= (1.0 - _clamp(launch_ease, 0.0, 1.0))
        ...
        try:
            ...
            pedal = 0.0
        return _clamp(pedal, 0.0, _HOLD_MAX_PEDAL)
        ...
        *,
        ...
        if self._state == STATE_STOPPING:
            ...
            if commanded_accel_ms2 > _HOLD_RELEASE_ACCEL_MS2:
                ...
                self._state = STATE_ROLLING
                self._rollback_decel_ms2 = 0.0
            elif abs(speed_kmh) < _HOLD_HOLDING_SPEED_KMH:
                # Truck is essentially stopped — transition into HOLDING. No
                # dwell required because the FF brake has already been running
                # since STOPPING engaged, so there is no transient on entry.
                self._state = STATE_HOLDING
                self._release_dwell_acc_s = 0.0
                self._launch_t = 0.0
        ...
        if self._state == STATE_LAUNCHING:
            ...
            # Settle to ROLLING when ramp completes and the truck is actually
            # moving in the intended direction (no rollback in progress).
            rolling_speed_ok = (
            ...
            if (
                ...
                # `t` fully retreated — drop back to HOLDING and re-enter the
                # release-dwell process. Integrator is preserved so we don't
                # forget what we learned about this slope.
                self._state = STATE_HOLDING
                self._release_dwell_acc_s = 0.0
                self._launch_t = 0.0
