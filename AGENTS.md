...
- **Ego/traffic pairing** (`core/radar/README.md` §16, `core/aeb/README.md` section 16): radar reads ego pose, speed and `simulatedTime` from the SCS telemetry block in the traffic buffer's game frame (`core/radar/scs_pose.py`), never from `telemetry_thread.data` while that block is readable. Replay gets frames from `core/aeb/clip_timebase.py`, which re-pairs schema 4 and older clips and rebuilds the simulated clock; do not feed `t_wall` to the reader directly. Exactly three physics steps is a sub-frame (`is_sub_frame`), which keeps the cadence the chain was tuned on.
...
- **AEB travel frame** (`core/aeb/README.md`, travel frame): regime flags (`head_on`, `near_head_on`, `co_directional`) take `fwd_dot` through `travel_sign(v.speed, cal)`, because target arcs already move along travel, and a reversing target is never oncoming. Both halves are load-bearing: heading alone missed a truck backing across ego's lane, travel alone handed a rig backing into it to the oncoming evasion rules. Velocity terms stay `v.speed * heading`; turn a travel-frame `fwd_dot` back into speed with `ctx.v_travel_speed`. The TMP rel-speed floor still applies to reversing targets: exempting them was tried and rejected for its low-speed FP cost.
- **AEB required decel** (`core/aeb/README.md` §5, "Clearance-based demand"): the demand is the minimum constant ego decel that keeps ego's front behind every body it would share space with, computed per target in `core/aeb/clearance.py` and aggregated by `max`. It is not the relative-frame `v_closing^2 / 2d`, and `v_closing` is not a braking requirement for anything off ego's axis: for a 90 degree crosser most of that vector is lateral and brakes do not steer. Do not re-add a "stop at the contact point" branch: braking cannot make ego arrive earlier, so the only escape braking buys is arriving behind, and a target that vacates the corridor stops constraining ego the moment it does. The co-directional limit reproduces `dv^2 / (2 * d_rel)` exactly and `tests/aeb/test_clearance.py` pins that, along with the stationary-obstacle and braking-lead equivalences: those three are the guarantee that rear-end timing and the whole stop-distance envelope did not move, so treat a failure there as a real regression rather than a tolerance to widen. `stop_buffer_response_s` is applied as a **time** lag, not a distance subtraction, which is what makes the co-directional limit come out exact while a crosser correctly pays `v0 * lag` instead of `v_closing * lag`. The lateral corridor test must stay ahead of the arc-length projection in `occupancy_profile`: calling `project_to_ego_arc` on every body point is 3.4x slower for an identical answer. Aggregate by `max` over targets, never by picking the lowest-`ttb` target: a crosser can now honestly report a low demand and would mask a slower-`ttb` lead behind it.
...
- **ACC standstill hold** (`core/acc/ACC_ARCHITECTURE.md` §10.1): released by the gap law's wanted accel in `core/cruise_control_thread/standstill_hold.py`, never by lead speed. `acc_speed`'s standstill latch reads a departing lead as 0 until ~1.35 m/s, which is exactly the lateness the old `v_lead < 0.4` release shipped. The guards are load-bearing together: the engage-time margin (a stop left at the hold FSM's own 0.25 re-launched on noise), no sub-launch bid at rest (the mapper's fast PI winds up against the hold brake), the unsmoothed snap from a positive cap (a cap decaying to 0 from above never trips the hold FSM's `commanded ≤ 0` capture, and a slow-rolling truck crept into the emergency band in closed loop), and the `min(raw, smoothed)` gap.
- **Capacity learning acceptance** (`core/sending_thread/README.md`): `PedalCapacityTracker` must never learn while the measured signal it is dividing by is still moving. Both sides now carry a settle gate on the signal itself, not just on the pedal: decel for `update_brake`, accel for `update_accel`. Do not lower `_GEAR_DWELL_S` back toward 0.30: the driveline takes about 1.5 s to restore torque after a shift, and a shorter dwell let the tracker read the torque interruption as truck weakness, drop the anchor, and step the gas pedal up 5-10% after every gear change. That defect survived several fixes aimed at the mapper's clutch freeze and capacity glide because it was never in `accel_to_pedals.py`. Re-measure with a replay over `accel_to_pedals_debug.csv` before touching either gate. The gas side is **affine**: at gas 0 in gear the sim still drives the truck (+0.50 m/s2, measured flat over 10-55 t, 5-35 m/s and the length of a coast), so `accel / pedal` is not a capability estimate and reading one out of a constant-speed cruise is what 75.9% of accepted samples used to be. Do not restore a single ratio through the origin, do not let the low-pedal band touch the slope, and do not drop the per-candidate reject again: it already went missing once, in `e151848`. The anchor is the pedal **slope**; full-pedal capability is `offset + slope` and `accel_gain_for_gear` owns that sum. The ratio's log-space step stays normalized by `1 + x^2`: `x` reaches -8 in top gear, and unnormalized it walked the (anchor, ratio) pair along its degenerate direction.
