...
def _accel_lane() -> Lane:
    return Lane(
        ...
        rules=[
            ...
            (T._ACC_SPEED_ACCEL_LO_MS2, "gate lo"),
            (T._ACC_SPEED_ACCEL_HI_MS2, "gate hi"),
        ],
        signals=[
            Curve("accel", f"2 acceleration (AEB), fit {T._ACCEL_FIT_WINDOW_S:g}s x scale",
                  _c(80, 225, 140), 1.7),
            Curve("acc_accel", f"2 acc_accel (ACC), fit {T._ACCEL_FIT_WINDOW_S:g}s x scale",
                  _c(240, 95, 80), 1.7),
            Curve("accel_trend",
                  f"step-4 gate trend {T._ACC_SPEED_ACCEL_WINDOW_S:g}s (fixed)",
                  _c(235, 175, 70), 1.2),
            Curve("accel_long",
...
def _gate_lane() -> Lane:
    return Lane(
        title="step 4 gates",
        short="step 4 gates",
        unit="0..1, tau x0.5, accel window x0.4",
        mode="fixed",
        lo=0.0,
        hi=1.05,
        height=110,
